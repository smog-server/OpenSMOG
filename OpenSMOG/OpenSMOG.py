...
To alleviate this instability, we allow one to truncate the Gaussian term at 4*sigma.  This threshold was chosen since it is comparable to the discretized and truncated Gaussian that is used in Gromacs.
    ...
    def loadSystem(self, Grofile="smog.gro", Topfile="smog.top", Xmlfile="smog.xml", noxml=False, Pdbfile=None):
        ...
        Args:
            ...
            Pdbfile (file, optional):
                PDB coordinate companion for a SMOG3 self-contained OpenSMOG
                XML file.  When this is supplied, OpenSMOG extracts embedded
                topology/coordinate data from ``Xmlfile`` and users do not need
                separate ``.top``, ``.gro``, ``.ndx`` or ``.contacts`` files.
        ...
        if Pdbfile is not None:
            return self.loadSMOG3System(Pdbfile=Pdbfile, Xmlfile=Xmlfile, noxml=noxml)
    ...
    def loadSMOG3System(self, Pdbfile, Xmlfile="smog.xml", noxml=False):
        ...
        if not os.path.exists(Pdbfile):
            SBM.opensmog_quit("Could not find PDB file {}".format(Pdbfile))
    ...
    def _parse_smog3_system_xml(self, Xmlfile):
        ...
        root = tree.getroot()
        payload = root.find("smog3_system")
        if payload is None:
            SBM.opensmog_quit(
                "XML file {} does not contain a smog3_system payload. "
                "Use loadSystem(Grofile=..., Topfile=..., Xmlfile=...) for "
    ...
    def _splitForces_contacts(self):
        ...
        for n in range(n_forces):
            forces[cont_data[4][n]] = [cont_data[0][n], cont_data[1][n], cont_data[2][n], cont_data[3][n]]
    ...
    def _customSmogForce(self, name, data, pbc):
        ...
        nonbond0=self.system.getForce(0)
        nonbond1=self.system.getForce(1)
        ...
        for iteraction in data[2]:
            ...
            # since exclusions are not in the top file, we need to add them here, based on the xml file
            if data[3]:
                if atom_index_i > atom_index_j:
                    index_j=atom_index_i
                    index_i=atom_index_j
                else:
                    index_i=atom_index_i
                    index_j=atom_index_j
                exlabel=f"{index_i}-{index_j}"
                if exlabel in self.bondedExclusions:
                    print(f"WARNING: Exclusion between atoms {int(index_i)+1} and {int(index_j)+1} is being generated by a contact, but it was also defined based on the top file (bond-generated, or listed under exclusions). Will skip this exclusion, to avoid raising an exception later.")
                elif exlabel in self.addedExclusions:
                    print(f"WARNING: Exclusion between atoms {int(index_i)+1} and {int(index_j)+1} appears in more than one contacts_type. Will skip this exclusion, to avoid raising an exception later.")
                else:
                    self.addedExclusions.add(f"{index_i}-{index_j}")
                    nonbond0.addException(index_i, index_j,chargeProd=0,sigma=0,epsilon=0,replace=True)
...
provided in the top and xml files.''')
                ...
                for i in range(len(contacts_xml)):
                    ...
                    tmp=[]                         
                    for generate in contacts_xml[i].iter('exclusions'):
                        tmp.append(generate.attrib['generate'])
                    if len(tmp) == 1:
                        if tmp[0] == "0":
                            addExclusions.append(False)
                        elif tmp[0] == "1":
                            addExclusions.append(True)
                        else:
                            SBM.opensmog_quit(f"Invalid exclusions generate value of {tmp[0]}") 
                    elif len(tmp) == 0:
                        addExclusions.append(False)
                ...
                xml_data['contacts']=[Expression,Parameters,Pairs,addExclusions,Force_Names]
...
''')
        ...
        if self.contacts_present==True: 
            ...
            self.bondedExclusions=set()
            original_customnonbonded=self.system.getForce(1)
            for i in range(original_customnonbonded.getNumExclusions()):
                exclusion_id = original_customnonbonded.getExclusionParticles(i)
                atom_index_i=exclusion_id[0]
                atom_index_j=exclusion_id[1]
                if atom_index_i > atom_index_j:
                    index_j=atom_index_i
                    index_i=atom_index_j
                else:
                    ...
                    index_j=atom_index_j
                exlabel=f"{index_i}-{index_j}"
                self.bondedExclusions.add(exlabel)
